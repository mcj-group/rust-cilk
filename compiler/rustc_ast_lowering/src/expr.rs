...
use super::errors::{
    ...
    BadControlFlowInCilkSpawn, ClosureCannotBeStatic, CoroutineTooManyParameters,
...
// For handling break, continue, and return statements inside cilk_for and cilk_spawn statements
...
impl<'a> CilkControlFlow<'a> {
    ...
    fn new_for_cilk_spawn(dcx: &'a DiagCtxtHandle<'a>) -> CilkControlFlow<'a> {
        CilkControlFlow { dcx, ctx: CilkControlFlowCtx::CilkSpawn, inner_labels: Vec::new() }
    }

    fn emit_bad_control_flow(&self, span: Span, keyword: &'static str) -> ExprKind {
        match self.ctx {
            CilkControlFlowCtx::CilkFor { .. } => ExprKind::Err(
                self.dcx.emit_err(BadControlFlowInCilkFor { span, keyword: keyword.to_owned() }),
            ),
            CilkControlFlowCtx::CilkSpawn => ExprKind::Err(
                self.dcx.emit_err(BadControlFlowInCilkSpawn { span, keyword: keyword.to_owned() }),
            ),
        }
    }
}
...
impl MutVisitor for CilkControlFlow<'_> {
    fn visit_expr(&mut self, e: &mut Expr) {
        ...
        match kind {
            ...
            ExprKind::Continue(label) => match self.ctx {
            ...
            ExprKind::ForLoop { iter, kind: ForLoopKind::CilkFor(_), .. } => {
...
impl<'hir> LoweringContext<'_, 'hir> {
    ...
    fn lower_expr_for(
        ...
        // Some(<pat>) => <body>,
        // For cilk_for we instead perform Some(<pat>) => cilk_spawn < call < closure <body> > >.
        let some_arm = {
            ...
            let body_expr = if let ForLoopKind::CilkFor(cilk_closure_node_id) = loop_kind {
                let induction_var_ident = match &ast_pat.kind {
                    ...
                    _ => {
                        let guar = self.dcx().has_errors().unwrap_or_else(|| {
                            self.dcx()
                                .span_err(ast_pat.span, "expected an identifier in `cilk_for`")
                        });
                ...
                // Place shadow_local_stmt right before body_block in the closure
                let closure_stmts = self.arena.alloc_from_iter(
                    std::iter::once(shadow_local_stmt).chain(body_block.stmts.iter().copied()),
                );
                let closure_body_block =
                    self.arena.alloc(hir::Block { stmts: closure_stmts, ..*body_block });
                ...
                let closure_block_expr = self.expr_block(closure_body_block);
                let body_id = self.lower_body(|_this| (&[], closure_block_expr));
                ...
                let closure_hir = self.arena.alloc(hir::Closure {
                    ...
                    constness: hir::Constness::NotConst,
                    move_ident: Some(induction_var_ident),
                });
                ...
                // Wrap the closure call in a cilk_spawn
                let call_stmt = self.stmt(body_block.span, hir::StmtKind::Semi(call_expr));
                let spawned_stmts = self.arena.alloc_from_iter([call_stmt]);
                let spawn_block = self.block_all(body_block.span, spawned_stmts, None);
                ...
                let outer_stmts_slice = self.arena.alloc_from_iter([spawn_stmt]);
