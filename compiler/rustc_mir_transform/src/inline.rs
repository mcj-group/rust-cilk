...
use rustc_hir::attrs::{InlineAttr, OptimizeAttr, OrphaningAttr};
...
impl<'tcx> MutVisitor<'tcx> for Integrator<'_, 'tcx> {
    ...
    fn visit_terminator(&mut self, terminator: &mut Terminator<'tcx>, loc: Location) {
        ...
        match terminator.kind {
            ...
            TerminatorKind::Reattach { sync_region: _, ref mut continuation } => {
                *continuation = self.map_block(*continuation);
            }
            TerminatorKind::Sync { sync_region: _, ref mut target } => {
                *target = self.map_block(*target);
            }
        }
    }
